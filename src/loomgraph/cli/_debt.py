...
def debt(codeindex_data: Path | None, output_format: str) -> None:
    ...
    """
    try:
        result = asyncio.run(_async_debt(codeindex_data, output_format))
...
async def _async_debt(
    codeindex_data_path: Path | None, output_format: str
) -> dict[str, Any]:
    ...
    report = await analyzer.analyze(codeindex_data=codeindex_data)
