...
- `import-export` / `index` summaries carry `test_entity_ratio` /
  ...
  (lh-enterprise measured 77% of edges from test files).
...
- `customers/customers.yaml.example` simplified (drops `lightrag_url` /
  ...
  none of them).
- `customers/DELIVERY_GUIDE.md` redacted: 6 plaintext GitHub PATs (zcyl /
  pinbianyi / demo) replaced with placeholders. **git history still contains
...
- Verified fabricOS: `graph src.__tests__.db-seed.test. --callees --depth 1`
...
- Verified on fabricOS: 544 ambiguous edges were 100% phantom before (all
