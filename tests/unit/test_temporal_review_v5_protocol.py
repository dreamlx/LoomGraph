"""V5 runner and adapter guards independent of historical cohorts."""
...
import pytest
from evals import audit_temporal_review_v5_pilot as audit
from evals import run_temporal_review_v5_pilot as pilot
from evals.deepswe import claude_orientation as orientation
from evals.temporal_review_v5_fixtures import contract
...
def _identity_probe(
    ...
    (root / "identity-preflight.json").write_text(json.dumps({
        "protocol": "temporal-review-v5-model-identity", "status": "complete", "identity_mode": mode,
...
def test_v5_command_schema_has_no_hidden_outcome_or_trust_or_call_cap() -> None:
    command = orientation.build_command(
        ...
        treatment_surface=orientation.TEMPORAL_REVIEW_V5_ADDITIVE_SURFACE, temporal_review_v5=True,
    )
...
def test_v5_condition_requirement_has_no_tool_call_cap() -> None:
    baseline = orientation._append_temporal_review_v5_requirement("review", "baseline")
    treatment = orientation._append_temporal_review_v5_requirement("review", "treatment")
...
def test_v5_packet_reports_six_calls_without_a_validity_failure(tmp_path: Path) -> None:
    task_id = "v5-working-tree-diff-default-navigation"
    _source(tmp_path)
    raw = _raw(task_id)
    raw_text = json.dumps(raw, separators=(",", ":"))
    tool_names = [orientation.TEMPORAL_MCP_TOOL, "Read", "Grep", "Grep", "Grep", "StructuredOutput"]

    packet = orientation.build_temporal_review_v5_packet(
...
def test_v5_audit_reports_six_calls_without_a_threshold_exclusion() -> None:
    tool_names = [orientation.TEMPORAL_MCP_TOOL, "Read", "Grep", "Grep", "Grep", "StructuredOutput"]
...
def test_v5_packet_binds_boundary_identity_and_one_pre_final_raw_event(tmp_path: Path) -> None:
    task_id = "v5-impact-caller-qualification-navigation"
    _source(tmp_path)
    raw = _raw(task_id)
    raw_text = json.dumps(raw, separators=(",", ":"))
    packet = orientation.build_temporal_review_v5_packet(
...
def test_v5_packet_stops_on_raw_after_final_result(tmp_path: Path) -> None:
    task_id = "v5-working-tree-diff-default-navigation"
    _source(tmp_path)
    packet = orientation.build_temporal_review_v5_packet(
        condition="treatment", use_mode="voluntary", source_clean=True, source_dir=tmp_path, return_code=0,
...
def test_v5_model_specific_identity_requires_exact_requested_model(tmp_path: Path) -> None:
...
def test_v5_runtime_identity_requires_each_cell_to_match_preflight() -> None:
...
def test_v5_model_identity_rebuilds_retained_stream(tmp_path: Path) -> None:
    probe = _identity_probe(
