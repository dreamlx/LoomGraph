...
if [[ $# -lt 2 || $# -gt 4 ]]; then
  echo "usage: $0 <baseline|treatment> <task-id> [jobs-dir] [voluntary|assisted]" >&2
  exit 2
fi
...
common_args=(
  ...
  --ak "orientation_use_mode=$use_mode"
...
PYTHONPATH="$repo_root/evals/deepswe:$deepswe_dir/agents${PYTHONPATH:+:$PYTHONPATH}" \
  ...
  --job-name "loomgraph-eval-$condition-$use_mode-$task_id"
