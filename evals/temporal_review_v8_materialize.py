"""Materialize an isolated source-only checkout for one V8 review cell."""
...
class V8MaterializationError(RuntimeError):
    """A frozen V8 source checkout could not be created safely."""


@dataclass(frozen=True)
class MaterializedTemporalReviewV8Fixture:
    task_id: str
    path: Path
    contract: Contract
...
def _git(path: Path | None, *args: str) -> str:
    ...
    if result.returncode:
        raise V8MaterializationError(result.stderr.strip() or "git command failed")
...
def _ensure_history(destination: Path, source: Path, item: Contract) -> None:
    ...
    if missing:
        ...
        except V8MaterializationError as exc:
            raise V8MaterializationError(
                "frozen V8 commits are absent and origin is unavailable"
            ) from exc
        _git(destination, "fetch", "--no-tags", origin, *missing)
    if any(not _has_commit(destination, sha) for sha in missing):
        raise V8MaterializationError("could not fetch all frozen V8 commits")


def materialize_temporal_review_v8_fixture(
    task_id: str, destination: Path, *, source_repository: Path
) -> MaterializedTemporalReviewV8Fixture:
    ...
    if (
        ...
        raise V8MaterializationError("materialized V8 checkout is not clean at frozen head")
    ...
    if not isinstance(exclusions, list) or any(
        ...
        raise V8MaterializationError("oracle-bearing path is visible or V8 exclusions are invalid")
    return MaterializedTemporalReviewV8Fixture(task_id, destination, item)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except (V8ContractError, V8MaterializationError) as exc:
