"""Materialize an isolated source-only checkout for one v4 review cell."""
...
class V4MaterializationError(RuntimeError):
    """A frozen v4 source checkout could not be created safely."""


@dataclass(frozen=True)
class MaterializedTemporalReviewV4Fixture:
    task_id: str
    path: Path
    contract: Contract
...
def _git(path: Path | None, *args: str) -> str:
    ...
    if result.returncode:
        raise V4MaterializationError(result.stderr.strip() or "git command failed")
...
def _ensure_history(destination: Path, source_repository: Path, item: Contract) -> None:
    ...
    except V4MaterializationError as exc:
        raise V4MaterializationError("frozen v4 commits are absent and origin is unavailable") from exc
    _git(destination, "fetch", "--no-tags", origin, *missing)
    if any(not _has_commit(destination, sha) for sha in missing):
        raise V4MaterializationError("could not fetch all frozen v4 commits")


def _validate_checkout(destination: Path, item: Contract) -> None:
    if _git(destination, "status", "--porcelain", "--untracked-files=all"):
        raise V4MaterializationError("materialized v4 checkout must start clean")
    if _git(destination, "rev-parse", "--abbrev-ref", "HEAD") != "HEAD":
        raise V4MaterializationError("materialized v4 checkout must be detached")
    if _git(destination, "rev-parse", "HEAD") != item.refs["head"]["commit_sha"]:
        raise V4MaterializationError("materialized v4 checkout is not at frozen head")
    for side in ("base", "head"):
        if _git(destination, "rev-parse", f"{item.refs[side]['alias']}^{{commit}}") != item.refs[side]["commit_sha"]:
            raise V4MaterializationError(f"v4 alias {item.refs[side]['alias']} is incorrect")
    exclusions = load_manifest().get("fixture_exclusion_globs")
    if not isinstance(exclusions, list):
        raise V4MaterializationError("v4 fixture exclusions are invalid")
    for pattern in exclusions:
        if not isinstance(pattern, str):
            raise V4MaterializationError("v4 fixture exclusions are invalid")
        if (destination / pattern.removesuffix("/**")).exists():
            raise V4MaterializationError(f"oracle-bearing path is visible: {pattern}")


def materialize_temporal_review_v4_fixture(
    task_id: str, destination: Path, *, source_repository: Path
) -> MaterializedTemporalReviewV4Fixture:
    ...
    if destination.exists():
        raise V4MaterializationError("destination must not already exist")
    if not (source_repository / ".git").exists():
        raise V4MaterializationError("source_repository is not a Git checkout")
    ...
    return MaterializedTemporalReviewV4Fixture(task_id, destination, item)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except (V4ContractError, V4MaterializationError) as exc:
