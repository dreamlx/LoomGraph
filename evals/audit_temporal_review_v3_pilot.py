"""Read-only audit for a complete, independent v3 temporal-review cohort."""
...
from evals import temporal_review_v3_fixtures as v3  # noqa: E402
from evals.run_temporal_review_v3_pilot import (  # noqa: E402
    MANIFEST_ID,
    PROTOCOL,
    SURFACE,
    expected_cells,
...
AUDIT_PROTOCOL = "temporal-review-v3-pilot-audit"
...
def _require_complete_root(root: Path) -> dict[str, Any]:
    if (root / "protocol-stop.json").exists():
        raise ValueError("refusing audit: v3 cohort has a protocol-stop record")
    design = _read(root / "preregistration.json")
    result = _read(root / "pilot-results.json")
    if not (root / "environment.json").is_file():
        raise ValueError("v3 environment artifact is missing")
    if design.get("protocol") != PROTOCOL or result.get("protocol") != PROTOCOL:
        raise ValueError("refusing non-v3 temporal-review cohort")
    if design.get("manifest_id") != MANIFEST_ID or design.get("surface") != SURFACE:
        raise ValueError("v3 preregistration identity is invalid")
    if design.get("mode") != "voluntary":
        raise ValueError("v3 cohort must be voluntary")
    expected = [
    ...
    if design.get("expected_cells") != expected:
        raise ValueError("v3 preregistration cell matrix was changed")
    manifest_hash = hashlib.sha256(v3.MANIFEST_PATH.read_bytes()).hexdigest()
    if design.get("manifest_sha256") != manifest_hash:
        raise ValueError("v3 manifest does not match preregistration hash")
    runs = result.get("runs")
    if not isinstance(runs, list) or len(runs) != len(expected):
        raise ValueError("v3 pilot result cardinality is invalid")
    ...
    if len(identities) != len(expected) or set(identities) != expected_identities:
        raise ValueError("v3 pilot result cells do not exactly equal preregistration")
    return design
        ...
        for item in message["content"]:
            ...
            for value in values:
                ...
                raw.append({
                    ...
                    "observation": v3.parse_raw_response(task_id, response),
                })
...
def _audit_cell(root: Path, task_id: str, replicate: int, condition: str) -> dict[str, Any]:
    ...
    if missing:
        raise ValueError(f"v3 cell lacks immutable artifacts: {missing}")
    ...
    if orientation.get("protocol") != SURFACE or driver.get("manifest_id") != MANIFEST_ID:
        raise ValueError(f"v3 cell has wrong protocol identity: {cell}")
    if (driver.get("task_id"), driver.get("replicate"), driver.get("condition")) != (task_id, replicate, condition):
        raise ValueError(f"v3 driver identity does not match path: {cell}")
    ...
    if driver.get("contract") != {
        ...
        raise ValueError(f"v3 driver contract does not match frozen task: {cell}")
    source = Path(driver.get("source_dir", ""))
    runner_command = driver.get("runner_command")
    if (
        not isinstance(runner_command, list)
        or "--temporal-review-v3-contract" not in runner_command
        or SURFACE not in runner_command
        or source.resolve() != (cell / "source").resolve()
    ):
        raise ValueError(f"v3 driver command or source path is invalid: {cell}")
    source_clean = (
    ...
    if source_clean:
        outcome = v3.evaluate_answer(task_id, answer, condition=condition, source_root=source, raw_response=selected["raw_response"] if selected else None)
    ...
    if condition == "treatment" and selected is not None and isinstance(warm, Mapping):
        ...
        if isinstance(warm_path, str) and Path(warm_path).is_file():
            try:
                ...
                warm_raw = None
            warm_observation = v3.parse_raw_response(task_id, warm_raw)
    ...
    valid_row = source_clean and runner_valid and tool_trace_valid and raw_trace_valid and orientation.get("status") == "complete" and semantic["passed"] is True and certificate_matches and (condition == "baseline" or selected is not None)
...
def _paired_deltas(records: list[dict[str, Any]]) -> dict[str, object]:
    ...
    for task_id in v3.TASK_IDS:
        for replicate in (1, 2):
            ...
            baseline_seconds, treatment_seconds = (
                baseline.get("agent_execution_seconds"), treatment.get("agent_execution_seconds")
            )
            if (
                baseline.get("status") != "valid"
                or treatment.get("status") != "valid"
                or not isinstance(baseline_seconds, (int, float))
                or not isinstance(treatment_seconds, (int, float))
            ):
                continue
            by_task.setdefault(task_id, []).append(
                {
                    "replicate": replicate,
                    "metric": "agent_execution_seconds",
                    "baseline": float(baseline_seconds),
                    "treatment": float(treatment_seconds),
                    "treatment_minus_baseline": float(treatment_seconds) - float(baseline_seconds),
                }
            )
    return {
        task_id: {
            "valid_pairs": rows,
            "summary": _quartiles(
                [cast(float, row["treatment_minus_baseline"]) for row in rows]
            ),
        }
...
def audit_pilot(output_root: Path) -> dict[str, object]:
    ...
    if discovered_paths != expected_paths:
        raise ValueError("v3 discovered cell artifacts do not exactly equal preregistration")
    ...
    result: dict[str, object] = {
        ...
        "runs": records,
        "paired_deltas_by_task": _paired_deltas(records),
    }
    path = root / "audited-results.json"
    if path.exists():
        raise ValueError("refusing to overwrite an existing v3 audit")
