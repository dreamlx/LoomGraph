"""Read-only audit for a complete, independent V5 temporal-review cohort."""
...
from evals import temporal_review_v5_fixtures as v5  # noqa: E402
from evals.deepswe import claude_orientation as orientation_runner  # noqa: E402
from evals.run_temporal_review_v5_pilot import (  # noqa: E402
    MANIFEST_ID,
    PROTOCOL,
    SURFACE,
    expected_cells,
...
AUDIT_PROTOCOL = "temporal-review-v5-pilot-audit"
...
def _require_complete_root(root: Path) -> dict[str, Any]:
    if (root / "protocol-stop.json").exists():
        raise ValueError("refusing audit: V5 cohort has a protocol-stop record")
    design = _read(root / "preregistration.json")
    result = _read(root / "pilot-results.json")
    if not (root / "environment.json").is_file():
        raise ValueError("V5 environment artifact is missing")
    if design.get("protocol") != PROTOCOL or result.get("protocol") != PROTOCOL:
        raise ValueError("refusing non-V5 temporal-review cohort")
    if design.get("manifest_id") != MANIFEST_ID or design.get("surface") != SURFACE:
        raise ValueError("V5 preregistration identity is invalid")
    if design.get("mode") != "voluntary":
        raise ValueError("V5 cohort must be voluntary")
    ...
    if (
        ...
        raise ValueError("V5 model identity is invalid")
    ...
    if (
        ...
        raise ValueError("V5 retained model identity artifact is invalid")
    retained_identity = _read(identity_path)
    stream_summary = orientation_runner.summarize_stream(_events(stream_path))
    if (
        retained_identity.get("protocol") != "temporal-review-v5-model-identity"
        ...
        raise ValueError("V5 retained model identity does not match preregistration")
    expected = [
    ...
    if design.get("expected_cells") != expected:
        raise ValueError("V5 preregistration cell matrix was changed")
    manifest_hash = hashlib.sha256(v5.MANIFEST_PATH.read_bytes()).hexdigest()
    if design.get("manifest_sha256") != manifest_hash:
        raise ValueError("V5 manifest does not match preregistration hash")
    if design.get("selection_preflight_sha256") != v5.selection_preflight_sha256():
        raise ValueError("V5 selection preflight does not match preregistration hash")
    runs = result.get("runs")
    if not isinstance(runs, list) or len(runs) != len(expected):
        raise ValueError("V5 pilot result cardinality is invalid")
    ...
    if len(identities) != len(expected) or set(identities) != expected_identities:
        raise ValueError("V5 pilot result cells do not exactly equal preregistration")
    return design
        ...
        for item in message["content"]:
            ...
            for value in values:
                ...
                raw.append({
                    ...
                    "observation": v5.parse_raw_response(task_id, response),
                })
...
def _audit_cell(root: Path, task_id: str, replicate: int, condition: str) -> dict[str, Any]:
    ...
    if missing:
        raise ValueError(f"V5 cell lacks immutable artifacts: {missing}")
    ...
    if orientation.get("protocol") != SURFACE or driver.get("manifest_id") != MANIFEST_ID:
        raise ValueError(f"V5 cell has wrong protocol identity: {cell}")
    if (driver.get("task_id"), driver.get("replicate"), driver.get("condition")) != (task_id, replicate, condition):
        raise ValueError(f"V5 driver identity does not match path: {cell}")
    ...
    if driver.get("contract") != {
        ...
        raise ValueError(f"V5 driver contract does not match frozen task: {cell}")
    source = Path(driver.get("source_dir", ""))
    runner_command = driver.get("runner_command")
    if (
        not isinstance(runner_command, list)
        or "--temporal-review-v5-contract" not in runner_command
        or SURFACE not in runner_command
        or source.resolve() != (cell / "source").resolve()
    ):
        raise ValueError(f"V5 driver command or source path is invalid: {cell}")
    source_clean = (
    ...
    if source_clean:
        outcome = v5.evaluate_answer(task_id, answer, condition=condition, source_root=source, raw_response=selected["raw_response"] if selected else None)
    ...
    if condition == "treatment" and selected is not None and isinstance(warm, Mapping):
        ...
        if isinstance(warm_path, str) and Path(warm_path).is_file():
            try:
                ...
                warm_raw = None
            warm_observation = v5.parse_raw_response(task_id, warm_raw)
...
def _paired_navigation_records(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    by_task: dict[str, list[dict[str, object]]] = {task_id: [] for task_id in v5.TASK_IDS}
...
def audit_pilot(output_root: Path) -> dict[str, object]:
    ...
    if discovered_paths != expected_paths:
        raise ValueError("V5 discovered cell artifacts do not exactly equal preregistration")
    ...
    if path.exists():
        raise ValueError("refusing to overwrite an existing V5 audit")
