"""Materialize an isolated source-only checkout for one v3 review cell."""
...
class V3MaterializationError(RuntimeError):
    """A frozen v3 source checkout could not be created safely."""


@dataclass(frozen=True)
class MaterializedTemporalReviewV3Fixture:
    task_id: str
    path: Path
    contract: Contract
...
def _git(path: Path | None, *args: str) -> str:
    ...
    if result.returncode:
        raise V3MaterializationError(result.stderr.strip() or "git command failed")
...
def _ensure_history(destination: Path, source_repository: Path, item: Contract) -> None:
    ...
    except V3MaterializationError as exc:
        raise V3MaterializationError("frozen v3 commits are absent and origin is unavailable") from exc
    _git(destination, "fetch", "--no-tags", origin, *missing)
    if any(not _has_commit(destination, sha) for sha in missing):
        raise V3MaterializationError("could not fetch all frozen v3 commits")


def _validate_checkout(destination: Path, item: Contract) -> None:
    if _git(destination, "status", "--porcelain", "--untracked-files=all"):
        raise V3MaterializationError("materialized v3 checkout must start clean")
    if _git(destination, "rev-parse", "--abbrev-ref", "HEAD") != "HEAD":
        raise V3MaterializationError("materialized v3 checkout must be detached")
    if _git(destination, "rev-parse", "HEAD") != item.refs["head"]["commit_sha"]:
        raise V3MaterializationError("materialized v3 checkout is not at frozen head")
    for side in ("base", "head"):
        if _git(destination, "rev-parse", f"{item.refs[side]['alias']}^{{commit}}") != item.refs[side]["commit_sha"]:
            raise V3MaterializationError(f"v3 alias {item.refs[side]['alias']} is incorrect")
    manifest = load_manifest()
    values = manifest.get("fixture_exclusion_globs")
    if not isinstance(values, list):
        raise V3MaterializationError("v3 fixture exclusions are invalid")
    for pattern in values:
        if not isinstance(pattern, str):
            raise V3MaterializationError("v3 fixture exclusions are invalid")
        if (destination / pattern.removesuffix("/**")).exists():
            raise V3MaterializationError(f"oracle-bearing path is visible: {pattern}")


def materialize_temporal_review_v3_fixture(
    task_id: str, destination: Path, *, source_repository: Path
) -> MaterializedTemporalReviewV3Fixture:
    ...
    if destination.exists():
        raise V3MaterializationError("destination must not already exist")
    if not (source_repository / ".git").exists():
        raise V3MaterializationError("source_repository is not a Git checkout")
    ...
    return MaterializedTemporalReviewV3Fixture(task_id, destination, item)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except (V3ContractError, V3MaterializationError) as exc:
