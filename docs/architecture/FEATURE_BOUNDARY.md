...
>
> **ADR-013（2026）已终结 LightRAG Fork 路线**：LoomGraph 改用 **SQLite + sqlite-vec** 作为存储后端（`~/.loomgraph/{workspace}.db`），不再依赖 LightRAG / PostgreSQL。Embedding 从 Jina Code V2（H200 TEI）改为默认本地 **Ollama**（`nomic-embed-text`），LLM 同走 Ollama（`gemma3:12b-it-qat`）。H200 服务器（`internal.example.invalid`）已于 2026-07 退役。
>
