...
```json
{
  ...
  "data": {
    "workspaces": [
      {"name": "zcyl-backend", "entities": 245, "relations": 1024},
      {"name": "zcyl-gateway", "entities": 89, "relations": 312},
      {"name": "zcyl-all", "entities": 380, "relations": 1520}
    ],
    "count": 3
  }
}
...
loomgraph workspace info zcyl-backend     # 指定 workspace
```
...
```bash
loomgraph workspace delete zcyl-backend:old-branch
```
