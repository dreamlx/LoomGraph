...
| B2 typed cross-module dependency | `python-deps`; `loomgraph deps -d 2 -w "$WS"` | **unsupported** | exactly one resolved `src/cli → src/core` aggregate with `CALLS: 1` and `IMPORTS: 1` | relation type, source scope, `partial`, resolution qualifier |
...
| C1 annotated-factory receiver adversary | `factory-receiver`; primary caller graph plus supplemental `loomgraph impact head --base base --depth 2 -w "$WS"` | **unsupported** | annotated positive caller is `consumer.run`; unannotated `consumer_unannotated.run` must not become a confirmed caller; sparse `only_here` change has zero callers but is `medium`/`unknown`, never isolated/low | source ID, resolution split, answer status, uncertainty reason |
| C2 barrel, alias, and dynamic receiver adversary | `ts-barrel-alias`; primary `src.alias_consumer` path-alias query plus supplemental `src.consumer` barrel query, then `src.alias_consumer.useAliasSession` CALLS query, all with `--include-unresolved` | **unsupported** | both references resolve to `src.models.Session`, never a ghost `src.index.Session`; dynamic `db.exec` remains an unresolved raw edge, never a synthetic `*.exec` target | workspace, backend, source ID, `edge_trust`, resolution split, qualifier, `dst_raw`, answer status |
...
6. **B2 scope:** the Python slice exercises resolved `CALLS` and resolved
   internal module-level `IMPORTS` aggregates. An `IMPORTS` endpoint must
   map uniquely to a source-bearing module; external, unresolved, ambiguous,
   or unmappable endpoints remain excluded rather than being inferred.
