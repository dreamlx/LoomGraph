# Temporal-review V5 re-registration

## Status and separation

V5 is a prospective cohort, independent of v1, v2, v3, v4, and every previous
...
pool v3 or v4. V4's stopped 6-cell root remains archive-only protocol-failure
evidence: it is never completed, rerun, scored, or combined with V5. Its two
tasks were selected by source review plus a deterministic, no-model raw
`branch-diff` preflight executed for this registration (2026-08-26 UTC):
...
[`evals/temporal-review-v5-fixture-manifest.json`](../../evals/temporal-review-v5-fixture-manifest.json).
The exact no-model raw selection responses and their hashes are retained in
[`evals/temporal-review-v5-selection-preflight.json`](../../evals/temporal-review-v5-selection-preflight.json)
...
The raw stream's full tool-call names and count are retained and independently
rebuilt by the audit for every run. Any call quantity is reporting metadata
only: it cannot make a run valid, invalid, excluded, or trigger cohort
expansion or stoppage. A mismatch between that reported metadata and the raw
stream is a trace-integrity exclusion. Unexpected MCP tools remain a hard
protocol failure; this change does not relax tool-surface or raw-evidence
trust.
