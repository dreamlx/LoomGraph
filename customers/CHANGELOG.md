...
- **孤儿二分类**:`truly_isolated`(真孤立,死代码信号,P1)vs `neighbors_unresolved`(有边但全部未解析——解析盲区,P2 低置信)。此前两类混在一起全是 P1
- **debt 空载不再满分**:无 codeindex 数据流入时 `maintainability` 输出 `null` 并按剩余维度重新加权,不再默认 100。总分口径因此变化(如 quality 100/topology 75:92→89)

### 行为变更 — `topology` 孤儿/枢纽/上帝函数指标改为按可解析图计算 (#149)

- **此前**：实体只要有任意一条存储的关系边(即使边的另一端从未解析成实体)就算「非孤儿」。这会系统性低估断连率——被判定「已连通」的实体，`loomgraph graph` 查询时可能返回空的 callers/callees。
- **现在**：只有两端都解析到实体的边才算连通，与 `graph` 的遍历口径一致。
- **影响**：上报的孤儿数会显著上升(更接近真实图谱质量)。Spring PetClinic 实测孤儿率 40.6%→79.9%;hub/god 数下降(悬挂边不再灌水度数，loomgraph 自身仓库 hub 15→7、god 44→3)。
...
- `graph()` 收了 `--depth` 但调 `_async_graph_query` 时丢了，导致 depth
  ...
  depth>1 传递性扩展，去重。
- fabricOS 实测：`graph src.__tests__.db-seed.test. --callees --depth 1` →
  22，`--depth 2` → 23（经一跳 callee 到达 `JSON.stringify`）。
- **注意**：密集代码图（如 fabricOS 5794 relations）depth>2 可能返回大量
  节点，按需选 depth。
